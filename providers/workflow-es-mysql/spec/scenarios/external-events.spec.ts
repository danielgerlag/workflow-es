...
import { spinWaitCallback, spinWait } from "../helpers/spin-wait";
 ...
 describe("external events", async () => {
    await createTestSchema();
     ...
     let workflowId = null;
     let instance = null;
     let persistence = new MySqlPersistence(getConnectionString());
     ...
     it("should be marked as complete", function(done) {
         expect(instance.status).toBe(WorkflowStatus.Complete);
         done();
     });

     it("should have return value of 'Pass'", function(done) {
